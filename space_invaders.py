import pygame
import os
import random

pygame.init()
WIDTH = 800
HEIGHT = 800
FPS = 60
SCREEN = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("Space Invaders")

# Enemy Laser
enemy_lasers = []
# Load assets
BACKGROUND = pygame.transform.scale(pygame.image.load("assets/background-black.png"),(WIDTH,HEIGHT))

# Ship img 
BLUE_SPACE_SHIP = pygame.image.load("assets/pixel_ship_blue_small.png")
GREEN_SPACE_SHIP = pygame.image.load("assets/pixel_ship_green_small.png")
RED_SPACE_SHIP = pygame.image.load("assets/pixel_ship_red_small.png")
YELLOW_SPACE_SHIP = pygame.image.load(os.path.join("assets","pixel_ship_yellow.png"))

# Laser img
YELLOW_LASER = pygame.image.load("assets/pixel_laser_yellow.png")
RED_LASER = pygame.image.load("assets/pixel_laser_red.png")
GREEN_LASER = pygame.image.load("assets/pixel_laser_green.png")
BLUE_LASER = pygame.image.load("assets/pixel_laser_blue.png")

class Ship:
    COOLDOWN = 30

    def __init__(self, x, y, health = 100):
        self.x = x
        self.y = y
        self.health = health
        self.ship_img = None
        self.laser_img = None
        self.lasers = []
        self.cool_down_counter = 0

    def draw(self, window):
        window.blit(self.ship_img, (self.x, self.y))

    def shoot(self):
        if self.cool_down_counter == 0:
            laser = Laser(self.x + self.ship_img.get_width() // 2 - self.laser_img.get_width() // 2, self.y, self.laser_img)
            self.lasers.append(laser)
            self.cool_down_counter = 1

    def cooldown(self):
        if self.cool_down_counter >= self.COOLDOWN:
            self.cool_down_counter = 0
        elif self.cool_down_counter > 0:
            if isinstance(self, Enemy):
                self.cool_down_counter += 0.5
            else:
                self.cool_down_counter += 1.5

    def get_width(self):
        return self.ship_img.get_width()
    
    def get_height(self):
        return self.ship_img.get_height()

class Player(Ship):
    def __init__(self, x, y, health = 100):
        super().__init__(x, y, health)
        self.ship_img = YELLOW_SPACE_SHIP
        self.laser_img = YELLOW_LASER
        self.max_health = 100
        self.mask = pygame.mask.from_surface(self.ship_img)

    def draw(self, window):
        super().draw(window)
        self.health_bar(window)

    def move_lasers(self, vel, objs):
        self.cooldown()
        for laser in self.lasers[:]:
            laser.move(vel)
            if laser.off_screen(HEIGHT): 
                self.lasers.remove(laser)
            for enemy in objs:
                    if collision(laser, enemy):
                        objs.remove(enemy)
                        if laser in self.lasers:
                            self.lasers.remove(laser)

    def health_bar(self, window):
        pygame.draw.rect(window, (255,0,0), (self.x, self.y + self.ship_img.get_height() + 10, self.ship_img.get_width(), 10))
        pygame.draw.rect(window, (0,255,0), (self.x, self.y + self.ship_img.get_height() + 10, self.ship_img.get_width() * (self.health/self.max_health), 10))

class Laser:
    def __init__(self, x, y, img):
        self.x = x
        self.y = y
        self.img = img
        self.mask = pygame.mask.from_surface(self.img)

    def move(self, vel):
        self.y += vel

    def draw(self, window):
        window.blit(self.img, (self.x, self.y))

    def off_screen(self, height):
        return not (0 <= self.y <= height)
    
def collision(obj1, obj2):
    offset_x = obj1.x - obj2.x
    offset_y = obj1.y - obj2.y
    return obj2.mask.overlap(obj1.mask, (offset_x, offset_y)) is not None
    
class Enemy(Ship):
    COLOR_MAP = {
        "red" : (RED_SPACE_SHIP, RED_LASER),
        "blue" : (BLUE_SPACE_SHIP, BLUE_LASER),
        "green" : (GREEN_SPACE_SHIP, GREEN_LASER)
    }

    def __init__(self, x, y , color):
        super().__init__(x, y)
        self.ship_img, self.laser_img = self.COLOR_MAP[color]
        self.mask = pygame.mask.from_surface(self.ship_img)
    
    def move(self, vel):
        self.y += vel

    def shoot(self):
        if self.cool_down_counter == 0:
            laser = Laser(self.x + self.ship_img.get_width() // 2 - self.laser_img.get_width() // 2, self.y + self.ship_img.get_height(), self.laser_img)
            enemy_lasers.append(laser)
            self.cool_down_counter = 1
            
    def move_lasers(self):
        self.cooldown()       

def main():
    clock = pygame.time.Clock()
    main_font = pygame.font.SysFont('comicsans',20)
    lost_font = pygame.font.SysFont('comicsans',30)

    level = 0
    lives = 5

    player = Player(WIDTH // 2 - RED_SPACE_SHIP.get_width()//2, HEIGHT - RED_SPACE_SHIP.get_height() - 50)
    player_velocity = 4

    enemy_laser_velocity = 2
    player_laser_velocity = 7
    lost = False
    lost_count = 0

    enemies = []
    enemy_velocity = 1
    wave = 5
    def lost_fun():
        nonlocal lost, lost_count, lives, level, wave, run, player, enemies, lost_font, clock
        while lost:
            clock.tick(60)
            lost_text = lost_font.render('You Lost!', 1, (255, 0, 0))
            SCREEN.blit(lost_text, (WIDTH // 2 - lost_text.get_width() // 2, HEIGHT // 2 - lost_text.get_height() // 2))
            restart_text = lost_font.render('Press R to Restart', 1, (255, 255, 255))
            SCREEN.blit(restart_text, (WIDTH // 2 - restart_text.get_width() // 2, HEIGHT // 2 + lost_text.get_height() // 2 + 10)) 

            pygame.display.update()
            
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    lost = False
                    run = False

            key = pygame.key.get_pressed()
            if key[pygame.K_r]:
                    lost = False
                    lost_count = 0
                    lives = 5  
                    level = 0
                    wave = 5
                    player = Player(WIDTH // 2 - RED_SPACE_SHIP.get_width()//2, HEIGHT - RED_SPACE_SHIP.get_height() - 50)
                    enemies.clear()
                    player.lasers.clear()
                    enemy_lasers.clear()
    def draw():
        SCREEN.blit(BACKGROUND, (0, 0))

        level_label = main_font.render(f"Level: {level}",1,(255,255,255))
        lives_label = main_font.render(f"Lives: {lives}",1,(255,255,255))
        
        SCREEN.blit(level_label, (10, 10))
        SCREEN.blit(lives_label, (WIDTH - lives_label.get_width() - 10, 10))

        for enemy in enemies:
            enemy.draw(SCREEN)

        player.draw(SCREEN)

        for laser in player.lasers:
            laser.draw(SCREEN)

        for laser in enemy_lasers:
            laser.draw(SCREEN)

        pygame.display.update()

    run = True
    while run:
        clock.tick(FPS)
        draw()
        # Check for game over conditions
        if lives <= 0 or player.health <= 0:
            lost = True
            lost_count += 1
        
        if lost:
            draw()
            lost_fun() 
        # No. of Space Invaders
        if len(enemies) == 0:
            level += 1
            wave += 5
            for _ in range(wave):
                enemy = Enemy(random.randrange(10, WIDTH - 100), random.randrange(-1500, -100), random.choice(["red", "blue", "green"]))
                enemies.append(enemy)

        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False

        key = pygame.key.get_pressed()
        if key[pygame.K_a] and player.x - player_velocity > 0:
            player.x -= player_velocity
        if key[pygame.K_d] and player.x + player_velocity + player.ship_img.get_width() < WIDTH:
            player.x += player_velocity
        if key[pygame.K_w] and player.y - player_velocity > 0:
            player.y -= player_velocity
        if key[pygame.K_s] and player.y + player_velocity + player.ship_img.get_height() < HEIGHT:
            player.y += player_velocity
        if key[pygame.K_SPACE]:
            player.shoot()
        
        

        for enemy in enemies:
            enemy.move(enemy_velocity)
            enemy.move_lasers()
            if collision(enemy, player):
                player.health -= 10
                enemies.remove(enemy)
            if enemy.y + enemy.get_height() > HEIGHT:
                lives -= 1
                enemies.remove(enemy)
            if random.randrange(0, 2 * FPS) == 1:
                enemy.shoot()

        for laser in enemy_lasers:
            laser.move(enemy_laser_velocity)
            if laser.off_screen(HEIGHT): 
                enemy_lasers.remove(laser)
            elif collision(laser, player):
                player.health -= 10
                enemy_lasers.remove(laser)

        player.move_lasers(-player_laser_velocity, enemies)
        for p_laser in player.lasers[:]:
            for e_laser in enemy_lasers[:]:
                if collision(p_laser, e_laser):
                    if p_laser in player.lasers:
                        player.lasers.remove(p_laser)
                    if e_laser in enemy_lasers:
                        enemy_lasers.remove(e_laser)
                    break
        pygame.display.flip()
    pygame.quit()

main()